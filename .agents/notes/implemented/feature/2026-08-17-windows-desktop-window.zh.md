...
HTTP 宿主仍绑定 loopback，因此窗口打开时浏览器也可以打开同一 URL。`loadURL` 之后，壳层取消后续的窗口内导航，并且不对被取消的主框架加载调用 `openExternal`（[自动化导航锁定](../bug-fix/2026-08-17-gui-automation-navigation-lock.md)）。用户点击的 `target=_blank` http(s) 链接在系统浏览器中对同一 URL 至多每五秒打开一次。该壳层不实现分层说明中点名的 IPC 载体、native 目录选择器提供方，或 `file://` dist 加载；那些仍属于未来的 Electron 客户端工作。
