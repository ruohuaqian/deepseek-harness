...
Host 的 `session.fork` RPC 接受 `atSeq`，并将其视为所需轮次内的锚点，而非 store 中包含该序号的安全边界。默认裁切（省略 `cut`）选择该锚点处或其后的首个 `turn/end`；锚点省略或超过末尾时，选择最后一个已完成轮次。若包容裁切的锚点已在日志中，但从该锚点起找不到匹配的 `turn/end`，则返回 `fork-unavailable`，绝不回退到更早的轮次，因此分支操作不会静默遗漏所点击的消息。`cut: 'before-turn'` 则把种子结束在包含必填 `atSeq` 的那一轮的 `turn/start` 之前，即使该轮仍在进行；因此重编辑已发送消息时可以丢掉这条提示词而不中止源会话。若此前没有任何轮次，则种子为空。

Host 通过 agent（智能体）注册表，以选定的种子和谱系创建子会话；发布前 setup 会先安装日志中最新的提供方、模型和推理（reasoning）目标，子会话才能运行。随后，Host 将子会话附加到源 Workspace。若附加失败，则返回 `workspace-attach-failed` 及已发布的子会话 id；客户端先将该子会话对账到摘要列表，再向调用方报告错误。Session 行操作使用最后一个已完成轮次，分支消息操作提供其事件 seq 做包容裁切，重编辑则带上 user 消息 seq 与 `cut: 'before-turn'`；Web 客户端随后归档源会话，使编辑覆盖当前列表项，而不是再嵌一套兄弟会话。
