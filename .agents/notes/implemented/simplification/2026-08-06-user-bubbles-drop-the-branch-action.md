...
The only fork handles are the enabled branch controls under settled answers. A turn cancelled before any node followed its message loses its only handle and has no fork point, matching turns whose tail is a content-free interrupted node. Web aria goldens across `apps/web` drop the user-bubble disabled-branch row and its hidden explanation text. Package tests pin that user and steering bubbles render no branch control and that a steering-tail turn leaves the narration's control unavailable. The seat left for branch-before-the-message is now occupied by sent-message Edit: confirm, rewind the current conversation to exclude that prompt, and prefill the composer ([sent-message re-edit](../feature/2026-08-17-sent-user-message-re-edit.md)).
