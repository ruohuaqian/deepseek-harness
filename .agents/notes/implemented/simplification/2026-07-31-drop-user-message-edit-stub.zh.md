...
Web 当时没有任何途径修正已发送的消息；从该消息分支是最接近的现有手势。由于动作行的内容完全由 props 组合而来，client 变更就绪后重新引入只是一次纯 UI 改动。该能力现已落地：已定稿 user 气泡上的编辑会在确认后把当前对话撤回到该轮之前，并把原文预填进输入框（[已发送消息重编辑](../feature/2026-08-17-sent-user-message-re-edit.md)）。
