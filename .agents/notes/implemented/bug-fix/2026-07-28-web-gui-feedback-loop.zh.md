...
提示词让 agent 而非用户负责隐藏的启动约定。HMR（热模块替换）接收端始终挂载，但客户端插件要自动重新加载，还需要在同一检出中运行 `pnpm run dev:web` 监听进程，agent 会在承诺无需刷新即可更新前验证这一点。外壳和其他普通包的变更仍然需要重新构建受影响的产物并刷新现有 URL。除非用户要求，agent 不会启动替代 GUI。它也不会用浏览器自动化或外部打开器去打开、绑定、导航或重载此 GUI URL——那会替换正在使用的页面；[自动化导航锁定](2026-08-17-gui-automation-navigation-lock.md)负责该禁令以及桌面窗 / SSE 机制。
