...
- **控制台隔离不可用。** 在受限令牌下，以 `CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` 创建的子进程在 DLL 初始化期间以 `STATUS_DLL_INIT_FAILED`（`0xC0000142`）死亡。POC 尝试把控制台登录 SID（`S-1-2-1`）加入 restricting 列表来修复；在 Windows 11 26200 上 `CreateWellKnownSid(WinLocalLogonSid)` 以 `ERROR_INVALID_PARAMETER`（87）失败，正确的 `WinConsoleLogonSid` 能产出合法 `S-1-2-1` 但子进程仍然死亡，POC 的最终修订同时移除了该 SID 与控制台隔离。子进程因此共享宿主控制台；stdio 重定向走管道，不受影响。加载 Win32 绑定会调用 `SetErrorMode`，因此该退出保持静默（不弹出 Application Error 对话框）；[沙箱分类器](../sandbox-local/)随后将 `0xC0000142` / `0xC0000022` 视为文件访问拒绝，以便工具提供 `sandbox_permissions` 升权。见 [静默启动失败 Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-17-windows-sandbox-silent-start-failure.md)。
