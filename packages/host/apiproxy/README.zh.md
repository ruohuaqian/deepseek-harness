...
`session.fork` 将可选事件锚点映射到已完成轮次前缀。默认裁切（省略 `cut`）是该锚点处或其后的首个 `turn/end`，使消息操作可包含该消息所在的完整轮次。锚点省略或超过末尾时，选择最后一个已完成轮次；若包容裁切的锚点已在日志中，而其所在轮次仍开放，则返回 `fork-unavailable`，不会向较早位置裁剪。`cut: 'before-turn'` 则把种子结束在包含必填 `atSeq` 的那一轮的 `turn/start` 之前，因此重编辑已发送消息时可以丢掉仍在进行或已完成的该轮，而不中止源会话。若此前没有任何轮次，则种子为空。发布后的子会话会先继承源会话的种子历史、cwd、日志中最新的 `ModelSelection` 及谱系，再加入源 Workspace。如果附加到 Workspace 失败，`workspace-attach-failed` 会携带已发布的子会话 id，供客户端对账。[SessionStore fork 决策](../../../.agents/notes/implemented/feature/2026-06-30-session-store-fork-api.md)记录了为何默认锚点要映射到该 `turn/end`；[已发送消息重编辑决策](../../../.agents/notes/implemented/feature/2026-08-17-sent-user-message-re-edit.md)记录了切在轮次之前的裁切。
