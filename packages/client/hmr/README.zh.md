...
- **对 `/plugins/events` 的文档导航返回 406** — 该通道只应答 `Accept: text/event-stream`。没有该头的浏览器 GET 不得挂在永不结束的 SSE 响应体上。
